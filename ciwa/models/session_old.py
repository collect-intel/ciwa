    ...
    async def collect_all_votes(self) -> None:
        ...
        for topic in self.topics:
            if not topic.voting_manager.is_labeling:
                start_time = time.time()
                task = asyncio.create_task(
                    self._collect_comparative_votes_with_logging(topic, start_time)
                )
                tasks.append(task)
        await asyncio.gather(*tasks)

    async def _collect_comparative_votes_with_logging(
        self, topic: Topic, start_time: float
    ) -> None:
        await topic.voting_manager.collect_votes(self.participants)
        logging.info(
            f"TIMING: Comparative votes collected for topic '{topic.title}' in {time.time() - start_time:.2f} seconds"
        )
