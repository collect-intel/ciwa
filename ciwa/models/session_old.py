...
class Session(Identifiable):
    ...
    async def gather_submissions_CURRENT(self) -> None:
        """
    ...
    async def gather_submissions(self) -> None:  # OLD
        """
        Gathers submissions from participants for each topic by using an asynchronous generator,
        allowing each submission to be processed as soon as it is generated.
        """
    ...
    async def create_submission_task(
        ...
        if submission is not None:
            ...
            if topic.voting_manager.is_label:
                await topic.voting_manager.collect_votes(submission, self.participants)

    # Deprecated by using create_submission_task, but this approach may be the better interface for delegating "return N submissions" to the Participant
        ...
        async with semaphore:
            ...
            async for submission in participant.generate_submissions(
                ...
                if submission is not None:
                    ...
                    if topic.voting_manager.is_label:
                        await topic.voting_manager.collect_votes(
    ...
    async def collect_all_votes(self) -> None:
        logging.info("Collecting any compare votes on topics.")
        tasks = []
        for topic in self.topics:
            if not topic.voting_manager.is_label:
                start_time = time.time()
                task = asyncio.create_task(
                    self._collect_compare_votes_with_logging(topic, start_time)
                )
                tasks.append(task)
        await asyncio.gather(*tasks)

    async def _collect_compare_votes_with_logging(
        self, topic: Topic, start_time: float
    ) -> None:
        await topic.voting_manager.collect_votes(self.participants)
        logging.info(
            f"TIMING: Compare votes collected for topic '{topic.title}' in {time.time() - start_time:.2f} seconds"
        )
    ...
    async def gather_results(self) -> None:
        ...
        for topic in self.topics:
            ...
            while not topic.submissions.empty():
                ...
                submission_data = submission.to_json()
